...
template <class type> HeapStructure<type>::HeapStructure(int nMax) {
    heap.push_back(mpz_class(-1));
    pos=0;
}
...
template <class type> inline void HeapStructure<type>::Insert(type data){
    heap.push_back(data);
    pos++;
    FixUp(pos);
}
